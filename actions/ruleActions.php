<?php
include 'models/RuleModel.php';

//RuleModel.php
	function addRule($params,$user_key) {
	...
	function getRule($params,$user_key) {
		//get all user
		if(authenticate($user_key)){
			$woodmodel = new WoodModel();
	...
	function editRule($params,$user_key) {
		if(authenticate($user_key)){
			if(empty($params))
				return array("error" => "1", "message" => "No Input Received!");
			$woodmodel = new WoodModel();
	...
	function deleteRule($params,$user_key) {
		if(authenticate($user_key)){
			if(empty($params))
				return array("error" => "1", "message" => "No Input Received!");
			$woodmodel = new WoodModel();
